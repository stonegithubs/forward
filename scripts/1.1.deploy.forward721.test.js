...
async function main() {
    ...
    if (
        ...
        const Forward20Imp = await ethers.getContractFactory(
            'Forward20Upgradeable'
        )
        const Forward721Imp = await ethers.getContractFactory(
            'Forward721Upgradeable'
        )
        const Forward1155Imp = await ethers.getContractFactory(
            'Forward1155Upgradeable'
        )
        const Factory20 = await ethers.getContractFactory(
            'Factory20Upgradeable'
        )
        const Factory721 = await ethers.getContractFactory(
            'Factory721Upgradeable'
        )
        const Factory1155 = await ethers.getContractFactory(
            'Factory1155Upgradeable'
        )
        ...
        // load pool
        // const forward20daiweth = await Forward20Imp.attach(
        //   config[network.name].deployed.forward20daiweth
        // )
        // const forward20wethdai = await Forward20Imp.attach(
        //   config[network.name].deployed.forward20wethdai
        // )
        const forward721nftdai = await Forward721Imp.attach(
            config[network.name].deployed.forward721nftdai
        )
        const forward721nftweth = await Forward721Imp.attach(
            config[network.name].deployed.forward721nftweth
        )
        // const forward1155sworddai = await Forward1155Imp.attach(
        //   config[network.name].deployed.forward1155sworddai
        // )
        // const forward1155swordweth = await Forward1155Imp.attach(
        ...
        let tx
        if (
            (await forward721nftdai.checkOrderState(orderId)).toString() == '0'
        ) {
            // not active
            ...
            console.log([deliveryPrice, buyerMargin, sellerMargin])
            tx = await forward721nftdai
                .connect(alice)
                .createOrderFor(
                    alice.address,
                    tokenIds,
        ...
        if (
            (await forward721nftdai.checkOrderState(orderId)).toString() == '1'
        ) {
            // active
            let buyerMargin = (await forward721nftdai.orders(orderId))
                .buyerMargin
            tx = await dai
                .connect(bob)
                .mint(buyerMargin, { gasLimit: network.config.gasLimit })
            // await tx.wait();
            tx = await dai
                .connect(bob)
                .approve(forward721nftdai.address, buyerMargin, {
                    gasLimit: network.config.gasLimit,
                })
            // await tx.wait();
            tx = await forward721nftdai
                .connect(bob)
                .takeOrderFor(bob.address, orderId, {
                    gasLimit: network.config.gasLimit,
                })
            let receipt = await tx.wait()
            console.log('txHash: ', receipt.transactionHash)
            console.log(
                'gasUsed-----takeOrderFor 721----: ',
                receipt.gasUsed.toString()
            )
            // await tx.wait();
        }
        let status = (
            await forward721nftdai.checkOrderState(orderId)
        ).toString()
        if (status == '2' || status == '4') {
            // filled => delivery
            let order = await forward721nftdai.orders(orderId)
            let deliverStart = order.deliverStart
            let now = (await web3.eth.getBlock(await web3.eth.getBlockNumber()))
                .timestamp
            console.log('deliver start at: ', deliverStart)
            console.log('now             : ', now)
            // seller deliver
            for (let i = 0; i < tokenIds.length; i++) {
                tx = await nft.connect(alice).mint(alice.address, tokenIds[i], {
                    gasLimit: network.config.gasLimit,
                })
                tx = await nft
                    .connect(alice)
                    .approve(forward721nftdai.address, tokenIds[i], {
                        gasLimit: network.config.gasLimit,
                    })
            }
            console.log('now             : ', now)
            now = (await web3.eth.getBlock(await web3.eth.getBlockNumber()))
                .timestamp
            if (deliverStart > now) {
                console.log(
                    'starting sleep for',
            ...
            tx = await forward721nftdai
                .connect(alice)
                .deliverFor(alice.address, orderId, {
                    gasLimit: network.config.gasLimit,
                })
            let receipt = await tx.wait()
            console.log('txHash: ', receipt.transactionHash)
            ...
            tx = await dai
                .connect(bob)
                .mint(deliveryPrice, { gasLimit: network.config.gasLimit })
            // await tx.wait();
            tx = await dai
                .connect(bob)
                .approve(forward721nftdai.address, deliveryPrice, {
                    gasLimit: network.config.gasLimit,
                })
            // await tx.wait();
            tx = await forward721nftdai
                .connect(bob)
                .deliverFor(bob.address, orderId, {
                    gasLimit: network.config.gasLimit,
                })
