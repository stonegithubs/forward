...
async function main() {
  ...
  if (
    ...
    let tx
    if ((await forward721nftdai.checkOrderState(orderId)).toString() == '0') {
      // not active
    ...
    if ((await forward721nftdai.checkOrderState(orderId)).toString() == '1') {
      // active
    ...
    if (status == '2' || status == '4') {
      ...
      tx = await forward721nftdai
        .connect(bob)
        .deliverFor(bob.address, orderId, { gasLimit: network.config.gasLimit })
