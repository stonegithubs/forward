...
async function main() {
    ...
    if (
        ...
        if (!config[network.name].deployed.hasOwnProperty("sword") || config[network.name].deployed.sword == "") {
        ...
        if (!config[network.name].deployed.hasOwnProperty("forward1155Imp") || config[network.name].deployed.forward1155Imp == "") {
        ...
        if (!config[network.name].deployed.hasOwnProperty("factory20") || config[network.name].deployed.factory20 == "") {
            const factory20 = await upgrades.deployProxy(
                ...
                [
                    ...
                    [config[network.name].deployed.weth, config[network.name].deployed.dai], 
                    deployer.address, 
                    10/* 10 /10000 */
                ],
                {
        ...
        if (!config[network.name].deployed.hasOwnProperty("factory1155") || config[network.name].deployed.factory1155 == "") {
        ...
        // deploy 20 pool
        if (!config[network.name].deployed.hasOwnProperty("forward20daiweth") || config[network.name].deployed["forward20daiweth"] == "") {
            const tx = await factory20.connect(deployer).deployPool(
                config[network.name].deployed.dai, 
                20, 
                config[network.name].deployed.weth
            );
            let receipt = await tx.wait();
            console.log('deploy forward20daiweth(asset:dai, margin:weth): ', receipt.events[1].args[3])
            config[network.name].deployed.forward20daiweth = receipt.events[1].args[3]
            utils.saveConfig(config);
        }

        if (!config[network.name].deployed.hasOwnProperty("forward20wethdai") || config[network.name].deployed["forward20wethdai"] == "") {
            const tx = await factory20.connect(deployer).deployPool(
                config[network.name].deployed.weth, 
                20, 
                config[network.name].deployed.dai
            ...
            let receipt = await tx.wait();
            console.log('deploy forward20wethdai(asset:weth, margin:dai): ', receipt.events[1].args[3])
            config[network.name].deployed.forward20wethdai = receipt.events[1].args[3]
            utils.saveConfig(config);
        }
        // deploy 721 pool
        if (!config[network.name].deployed.hasOwnProperty("forward721nftdai") || config[network.name].deployed["forward721nftdai"] == "") {
            const tx = await factory721.connect(deployer).deployPool(
                config[network.name].deployed.nft, 
                721, 
                config[network.name].deployed.dai
            );
            let receipt = await tx.wait();
            console.log('deploy forward721nftdai(asset:nft, margin:dai): ', receipt.events[1].args[3])
            config[network.name].deployed.forward721nftdai = receipt.events[1].args[3]
            utils.saveConfig(config);
        }
        if (!config[network.name].deployed.hasOwnProperty("forward721nftweth") || config[network.name].deployed["forward721nftweth"] == "") {
            const tx = await factory721.connect(deployer).deployPool(
                config[network.name].deployed.nft, 
                721, 
                config[network.name].deployed.weth
            ...
            let receipt = await tx.wait();
            console.log('deploy forward721nftweth(asset:nft, margin:weth): ', receipt.events[1].args[3])
            config[network.name].deployed.forward721nftweth = receipt.events[1].args[3]
            utils.saveConfig(config);
        }
        // deploy 1155 pool
        if (!config[network.name].deployed.hasOwnProperty("forward1155sworddai") || config[network.name].deployed["forward1155sworddai"] == "") {
            const tx = await factory1155.connect(deployer).deployPool(
                config[network.name].deployed.sword, 
                1155, 
                config[network.name].deployed.dai
            );
            let receipt = await tx.wait();
            console.log('deploy forward1155sworddai(asset:sword, margin:dai): ', receipt.events[1].args[3])
            config[network.name].deployed.forward1155sworddai = receipt.events[1].args[3]
            utils.saveConfig(config);
        }
        if (!config[network.name].deployed.hasOwnProperty("forward1155swordweth") || config[network.name].deployed["forward1155swordweth"] == "") {
            const tx = await factory1155.connect(deployer).deployPool(
                config[network.name].deployed.sword, 
                1155, 
                config[network.name].deployed.weth
            ...
            let receipt = await tx.wait();
            console.log('deploy forward1155swordweth(asset:sword, margin:weth): ', receipt.events[1].args[3])
            config[network.name].deployed.forward1155swordweth = receipt.events[1].args[3]
            utils.saveConfig(config);
        }
