...
describe('Forward721 TestCase with marginToken as ERC20', function () {
    ...
    beforeEach(async () => {
        ...
        await this.factory721
            .connect(this.alice)
            .deployPool(this.nft.address, 721, this.dai.address)
    ...
    it('should deployPool correctly', async () => {
        ...
        expect(
            await this.factory721.getPair(this.nft.address, this.dai.address)
    ...
    it('should createOrder correctly', async () => {
        ...
        await this.dai
            .connect(this.alice)
            .approve(this.forward721.address, sellerMargin)
        await this.forward721
            .connect(this.alice)
            .createOrderFor(
                this.alice.address,
                tokenIds,
                [orderValidPeriod, deliverStart.toString(), deliveryPeriod],
    ...
    it('should NOT take order', async () => {
        ...
        await network.provider.send('evm_increaseTime', [
            orderValidPeriod + 100,
        ])
        await network.provider.send('evm_mine')
        await expectRevert(
            this.forward721
                .connect(this.bob)
                .takeOrderFor(this.alice.address, 0),
            '!active'
        )
        expect((await this.forward721.checkOrderState(0)).toString()).to.equal(
            '3'
        ) // dead
    })
    it('should take order correctly', async () => {
        ...
        expect(
            (await this.dai.balanceOf(this.bob.address)).toString()
        ).to.equal(buyerMargin)
        await this.dai
            .connect(this.bob)
            .approve(this.forward721.address, buyerMargin)
        expect(
            (
                await this.dai.allowance(
                    this.bob.address,
        ...
        await this.forward721
            .connect(this.bob)
            .takeOrderFor(this.bob.address, 0)
        let order = await this.forward721.orders(0)
    ...
    it('should deliver correctly for both seller and buyer', async () => {
        ...
        for (let i = 0; i < tokenIds.length; i++) {
            await this.nft
                .connect(this.alice)
                .mint(this.alice.address, tokenIds[i])
            await this.nft
                .connect(this.alice)
                .approve(this.forward721.address, tokenIds[i])
        }
        await this.forward721
            .connect(this.alice)
        ...
        await this.dai
            .connect(this.bob)
            .approve(this.forward721.address, deliveryPrice)
        ...
        let cfee = new BN(deliveryPrice)
            .mul(new BN(2))
            .mul(this.opFee)
            .divn(new BN(this.base)) // * 2 means taking fee from both sides
        // console.log("cfee       : ", cfee.toString())
        // console.log("actual cfee: ", (await this.forward721.cfee()).toString())
        expect((await this.forward721.cfee()).toString()).to.equal(
            cfee.toString()
        )
    })
    ...
    it('Should invoke factory.collectFee correctly', async () => {
        ...
        await expectRevert(
            this.factory721
                .connect(this.bob)
                .collectFee(this.alice.address, [0]),
            'Ownable: caller is not the owner'
        )
        await this.factory721
            .connect(this.alice)
            .collectFee(this.carol.address, ['0'])
        expect(
            (await this.dai.balanceOf(this.carol.address)).toString()
        ).to.equal(cfee0.toString())
    })
    it('Should invoke factory.withdrawOther successfully', async () => {
        ...
        await expectRevert(
            this.factory721
                .connect(this.bob)
                .withdrawOther(0, dai1.address, this.carol.address),
            '!auth'
        )
        await this.factory721
            .connect(this.alice)
            .withdrawOther(0, dai1.address, this.carol.address)
        expect((await dai1.balanceOf(this.carol.address)).toString()).to.equal(
            '100'
        )
    })
    it('Should invoke factory.pausePools and unpausePools correctly', async () => {
        await expectRevert(
            this.factory721.connect(this.bob).pausePools([0]),
            'Ownable: caller is not the owner'
        )
        ...
        expect((await this.forward721.ordersLength()).toString()).to.equal('0')
        {
            ...
            await this.dai
                .connect(this.alice)
                .approve(this.forward721.address, sellerMargin)
            const tx = await this.forward721
                .connect(this.alice)
                .createOrderFor(
                    this.alice.address,
                    tokenIds,
            ...
            console.log(
                'gasLimit-----createOrder----: ',
                tx.gasLimit.toString()
            ) // 37.2.8k better than archeNetwork
        }
        expect((await this.forward721.ordersLength()).toString()).to.equal('1')
    })
    it('Should takeOrder, deliver, settle correctly if hVault is enabled', async () => {
        {
            await expectRevert(
                this.forward721
                    .connect(this.bob)
                    .setForwardVault(this.fVault.address),
                '!factory'
            )
            await this.factory721
                .connect(this.alice)
                .setForwardVault(0, this.fVault.address)
            ...
            await this.dai
                .connect(this.alice)
                .approve(this.yVault.address, toWei('100'))
            await this.yVault.deposit(toWei('100'))
            // yVault(b:100, t:100)
            expect(
                (await this.yVault.getPricePerFullShare()).toString()
            ).to.equal(toWei('1', 'ether'))
            await this.dai.connect(this.alice).mint(toWei('1'))
            await this.dai
                .connect(this.alice)
                .transfer(this.yVault.address, toWei('1'))
            expect(
                (await this.yVault.getPricePerFullShare()).toString()
            ).to.equal(toWei('1.01', 'ether'))
            // yVault(b:101, t:100)
        }
        ...
        let now = await time.latest()
        {
            ...
            let fVaultPrice
            {
                ...
                yVaultSupply = toWeiBN('117')
                    .mul(toWeiBN('100'))
                    .div(toWeiBN('101'))
                yVaultPrice = toWeiBN('118')
                    .mul(toWeiBN('101'))
                    .mul(toWeiBN('1'))
                    .div(toWeiBN('117'))
                    .div(toWeiBN('100'))
                expect(
                    (await this.yVault.getPricePerFullShare()).toString()
                ).to.equal(yVaultPrice.toString())
                hVaultSharesInY = toWeiBN('117')
                    .mul(toWeiBN('100'))
                    .div(toWeiBN('101'))
                    .sub(toWeiBN('100'))
                expect(
                    (
                        await this.yVault.balanceOf(this.fVault.address)
                    ).toString()
                ).to.equal(hVaultSharesInY.toString())
                fVaultPrice = hVaultSharesInY
                    .mul(yVaultPrice)
                    .div(toWeiBN('1'))
                    .add(toWeiBN('4'))
                    .mul(toWeiBN('1'))
                    .div(toWeiBN('20'))
                expect(
                    (await this.fVault.getPricePerFullShare()).toString()
                ).to.equal(fVaultPrice.toString())
                expect(
                    (await this.forward721.getPricePerFullShare()).toString()
                ).to.equal(fVaultPrice.toString())
                fVaultSupply = toBN(
                    (await this.fVault.totalSupply()).toString()
                )
            }
            // buyer take order
            let buyerShare
            {
                ...
                expect(
                    equals(
                        toBN(
                            (
                                await this.forward721.orders(0)
                            ).buyerShare.toString()
                        ),
                        buyerShare
                    )
