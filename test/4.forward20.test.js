...
describe("Forward20 TestCase with marginToken", function() {
    ...
    it("should deliver correctly for both seller and buyer", async() => {
        ...
        await this.forward20.connect(this.alice).createOrderFor(
            this.alice.address,
            amount,
            [orderValidPeriod,
            deliveryStart,
        ...
        await this.want.connect(this.alice).approve(this.forward20.address, amount)
        await this.forward20.connect(this.alice).deliverFor(this.alice.address, 0);
        order = await this.forward20.orders(0);
